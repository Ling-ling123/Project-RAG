...
def load_text_file(file_path):
    ...
    try:
        ...
        if not content.strip():
            return None, f"文本文件 {file_path} 内容为空"
...
def load_docx_file(file_path):
    ...
    try:
        ...
        # 使用生成器处理文本，避免一次性加载所有内容
        content = '\n'.join(process_paragraphs())
        if not content.strip():
            return None, f"Word文档 {file_path} 内容为空"
        return clean_text(content), None
    except Exception as e:
...
def load_markdown_file(file_path):
    ...
    try:
        ...
        if not md_content.strip():
            return None, f"Markdown文件 {file_path} 内容为空"
        ...
        if not content.strip():
            return None, f"Markdown文件 {file_path} 转换后内容为空"
...
def load_documents_from_directory(directory_path, file_types=None):
    ...
    if not os.path.exists(directory_path):
        error_msg = f"目录 {directory_path} 不存在"
    ...
    for file_type in file_types:
        ...
        for file_path in file_paths:
            ...
            if error:
                errors.append(error)
                logging.error(f"加载文件失败: {error}")

    # 如果没有成功加载任何文档
    if not documents:
        error_msg = f"在目录 {directory_path} 中未找到任何可处理的文档"
        errors.append(error_msg)
        logging.error(error_msg)
...
def create_example_files(directory_path):
    ...
    with open(os.path.join(directory_path, "example.txt"), "w", encoding="utf-8") as f:
        f.write(
            "这是一个示例文档，用于测试文档加载功能，界面新闻是中国具有影响力的原创财经新媒体，由上海报业集团出品，2014年9月创立。界面新闻客户端曾被中央网信办评为App影响力十佳。2017—2022年位居艾瑞商业资讯类移动App指数第一名")
